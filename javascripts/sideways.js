...
$(window).load(function() {
	...
	function CustomScroller(){
		...
		if($customScrollBox_container.height()>visibleHeight){ //custom scroll depends on content height
			...
			function DraggerOver(){
				$dragger.css("background", "#E67E22");
			}

			function DraggerOut(){
				$dragger.css("background", "#D35400");
			}
	...
	$toolbar.stop().fadeTo("fast",0.7); //set its original state
	...
	function SlidePanels(action){
		...
		if(action=="open"){
			...
			$bg.stop().animate({left: 140}, speed,easing);
		} else {
			$outer_container.stop().animate({left: -700}, speed,easing);
			$bg.stop().animate({left: 140}, speed,easing,function(){$("#arrow_indicator").fadeTo("fast",1);});
		}
	}
